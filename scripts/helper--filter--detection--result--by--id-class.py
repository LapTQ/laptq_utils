...
import shutil
import subprocess
from laptq_pyutils.helper import helper__filter__detection__result__by__id_class
...
def run_wrapper(kwargs):
    ...
    helper__filter__detection__result__by__id_class(**kwargs)
    
    print(f"{TAG__PASSED} Done processing: {kwargs['path__dir__lbl__input']}")

    # Verification Logic
    # Note: os.listdir counts all entries (including subdirs if any), similar to 'find' logic in flat dirs
