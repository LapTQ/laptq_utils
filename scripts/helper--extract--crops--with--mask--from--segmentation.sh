...
for subpath__dir in "${!MAP__SUBPATH_DIR__TO__[@]}"; do
    ...
    path__dir__crop__img__output="${PATH__DIR__OUTPUT}/${subpath__dir}/crops${POSTFIX__DIR__OUTPUT}"
    path__dir__crop__mask__output="${PATH__DIR__OUTPUT}/${subpath__dir}/masks${POSTFIX__DIR__OUTPUT}"
    ...
    [[ -d "${path__dir__crop__mask__output}" ]] && rm -r "${path__dir__crop__mask__output}"
    [[ -d "${path__dir__crop__lbl__output}" ]] && rm -r "${path__dir__crop__lbl__output}"
    mkdir -p "${path__dir__crop__img__output}"
    mkdir -p "${path__dir__crop__mask__output}"
    ...
    python3 submodules/laptq_utils/main.py \
        ...
        --path__dir__crop__mask__output "${path__dir__crop__mask__output}" \
