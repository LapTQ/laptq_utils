...
MAP__SUBPATH_DIR__TO__ = {
    ...
    for p in glob.glob(f"{PATHD_IMAGE}/shoplifting-awljp-demo-videos/demo_room/*.mp4")
    if os.path.isdir(p)
}
...
import shutil
import subprocess
from laptq_pyutils.helper import helper__extract__topdown__pose
...
def run_wrapper(kwargs):
    ...
    helper__extract__topdown__pose(**kwargs)
...
for subpath__dir in MAP__SUBPATH_DIR__TO__:
    path__dir__img__input = f"{PATHD_IMAGE}/{subpath__dir}/images{POSTFIX_IMAGE}"
    path__dir__lbl__input = (
        f"{PATHD_LABEL_INPUT}/{subpath__dir}/labels{POSTFIX_LABEL_INPUT}"
    )
    path__dir__lbl__output = (
        f"{PATHD_LABEL_OUTPUT}/{subpath__dir}/labels{POSTFIX_LABEL_OUTPUT}"
    )
